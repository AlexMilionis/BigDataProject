...
spark = SparkSession.builder \
    .appName("query3_catalyst_optimizer") \
    .getOrCreate()

path = 'hdfs://master:9000/datasets/'

# since we only need for 2015, we read the first csv
df      = spark.read.csv(path + "crime_data.csv",     header=True)
zips    = spark.read.csv(path + "revgecoding.csv",    header=True)
incomes = spark.read.csv(path + "LA_income_2015.csv", header=True)
...
print('===========================')
df_zips = df.join(zips, on=['LAT', 'LON'], how='inner')
...
df_zips_income = df_zips.join(incomes, on=['ZIP Code'], how='inner')
print('===========================')
